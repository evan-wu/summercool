...
/**
 * 取得页面多国语言的message信息函数
 * 
 * @author:shaochuan.wangsc
 * @date:2010-3-10
 * 
 */
public class FreeMarkerMessageFunction implements TemplateMethodModel {
	...
	public Object exec(List arguments) throws TemplateModelException {
		...
		WebApplicationContext wac = RequestContextUtils.getWebApplicationContext(servletAttrs.getRequest());
		...
		if (FreeMarker.SUPPORTED_LANGUAGE_MAP.get(firstArgument) != null) {
			...
			result = wac.getMessage(secondArgument, args.toArray(), localeBean.getLocal());
		} else {
			List<Object> args = new ArrayList<Object>();
			if (arguments.size() > 1) {
			...
			result = wac.getMessage(firstArgument, args.toArray(), locale);
		}
		return result;
	}
}
