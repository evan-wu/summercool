package com.bdconsulting.summercool.web.context;
...
import com.bdconsulting.summercool.web.util.BeanUtils;
...
public class ClassPathControllerAndWidgetScanner {
	...
	public static final Log logger = LogFactory.getLog(ClassPathControllerAndWidgetScanner.class);
			...
			for (Resource resource : resources) {
				if (resource.isReadable()) {
					...
					if (classMetadata.isConcrete()) {
						...
						Object clazz = beanFactory.createBean(classLoader.loadClass(className), AutowireCapableBeanFactory.AUTOWIRE_NO, false);
						//
						...
						if(clazz instanceof Controller){
							...
							String controllerKey = BeanUtils.makeControllerOrWidgetMappingKey(controller) + webModuleConfigurer.getUriExtension();
							String controllerRule = webModuleConfigurer.getContextPackage() + CONTROLLERS_PACKAGE + SLASH;
							if(controllerKey.startsWith(controllerRule)){
								controllerKey = controllerKey.replaceFirst(controllerRule, webModuleConfigurer.getContext());
								webModuleConfigurer.getControllerMap().put(controllerKey, (Controller)clazz);
								controllers.put(controllerKey, (Controller)clazz);
							}
						}
					}
				}
